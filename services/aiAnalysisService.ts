...
export async function analyzeTrashImage(_uri: string): Promise<{ isTrash: boolean; trashType: TrashType; confidence: number }> {
  await new Promise(resolve => setTimeout(resolve, 2200));
  const isTrash = Math.random() > 0.18;
  return { isTrash, trashType: choices[Math.floor(Math.random() * choices.length)], confidence: 0.86 + Math.random() * 0.12 };
}
